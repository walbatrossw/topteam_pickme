										...
										<c:forEach var="s" items="${oneDay.scheduleList}">
											<div>
												...
												<c:if test="${s.recruitName.length()<=9}">
													<a href="/recruitDetail?recruitCompanyCd=${s.recruitCompanyCd}" data-toggle="modal" data-target="#datailModal">${s.recruitName}</a>
													
												</c:if>
												<c:if test="${s.recruitName.length()>9}">
													<a href="/recruitDetail?recruitCompanyCd=${s.recruitCompanyCd}" data-toggle="modal" data-target="#detailModal">${fn:substring(s.recruitName,0,9)}</a>
												</c:if>
											</div>
										</c:forEach>
									</c:if>
